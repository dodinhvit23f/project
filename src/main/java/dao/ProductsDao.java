...
public class ProductsDao extends BaseDao<Products> {
	...
	protected void setChange(Products oldClazz, Products newClazz) {
        ...
        public List<Products> selectLimit (int  start,int limmit ){
		this.session = Hibernate.getConnection();
		List<Products> list = this.session
				.createNativeQuery("select * from products" + "  LIMIT ?,? ", Products.class)
                                .setParameter(1, start)
                                .setParameter(2, limmit)
				.getResultList();
		Utility.closeObject(session);
		return list;
        }
