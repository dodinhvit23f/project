...
public class BillsHome {
	...
	public Bills merge(Bills detachedInstance) {
		...
		try {
			Bills result = (Bills) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Bills findById(java.lang.Integer id) {
		...
		try {
			Bills instance = (Bills) sessionFactory.getCurrentSession().get("entity.Bills", id);
			if (instance == null) {
	...
	public List findByExample(Bills instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("entity.Bills")
